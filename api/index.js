...
app.get("/certificados/:id", async (req, res) => {
    ...
    if (!id) {
        return res.status(400).send("ID do certificado não fornecido");
    }
    ...
    try {
        ...
        if (redisData) {
            console.log("Certificado encontrado no cache Redis");
        ...
        if (result.rows.length === 0) {
            return res.status(404).send("Certificado não encontrado para o ID especificado");
        }

        const caminhoCertificado = result.rows[0].caminho_certificado;

        await clientRedis.setEx(redisKey, 3600, JSON.stringify(caminhoCertificado));
